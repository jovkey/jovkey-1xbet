...
export default function Navbar() {
  ...
  return (
    ...
    <nav className="fixed top-0 left-0 w-full z-50 glass px-3 sm:px-4 xl:px-6 py-3 xl:py-4">
      <div className="flex justify-between items-center gap-2 sm:gap-3">
        <div className="min-w-0 leading-none">
          <div className="text-base min-[360px]:text-lg sm:text-xl xl:text-2xl font-black tracking-tighter text-gold italic whitespace-nowrap">
            Coupon<span className="text-white"> Gratuit</span>
          </div>
          <div className="text-[8px] sm:text-[9px] xl:text-[10px] uppercase tracking-[0.25em] text-gray-400 font-semibold mt-0.5 whitespace-nowrap">de JovGroup</div>
        </div>
        <div className="hidden xl:flex space-x-6 font-medium uppercase text-sm tracking-widest">
          {LINKS.map((l) => (
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <button
            onClick={copyPromo}
            className="gold-gradient px-2 min-[360px]:px-2.5 sm:px-3 xl:px-6 py-2 rounded-full font-bold text-[10px] min-[360px]:text-[11px] sm:text-xs xl:text-sm shadow-lg hover:brightness-110 transition tap-target whitespace-nowrap"
          >
            CODE : {PROMO_CODE}
          </button>
          <button
            ...
            className={`xl:hidden shrink-0 rounded-xl p-2.5 sm:p-3 tap-target shadow-lg transition ${
      ...
      {open && (
        <div className="xl:hidden absolute left-0 right-0 top-full mt-2 mx-3 p-3 rounded-2xl bg-night border border-white/10 shadow-2xl flex flex-col gap-2 font-black uppercase text-base tracking-wide z-50 max-h-[calc(100dvh-90px)] overflow-y-auto overscroll-contain">
